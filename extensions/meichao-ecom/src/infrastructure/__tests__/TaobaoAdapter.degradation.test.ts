...
describe("TaobaoAdapter - Detailed Degradation Flow", () => {
  ...
  describe("Scenario 6: Custom source configuration", () => {
    it("should use custom primary source from sourceConfig", async () => {
      logStage("INIT", {
        scenario: "Custom source config",
        config: { primary: "taobao_third_party", fallbacks: ["taobao_crawler"] },
      });
      ...
      await logDataSourceStatus(customAdapter);
    ...
    it("should only use configured fallbacks, skip non-configured sources", async () => {
      logStage("INIT", {
        scenario: "Only configured fallbacks",
        config: { primary: "taobao_official_api", fallbacks: ["taobao_crawler"] },
      });
    ...
    it("should fail when all configured sources are unavailable", async () => {
      logStage("INIT", {
        scenario: "All configured sources fail",
        config: { primary: "taobao_official_api", fallbacks: ["taobao_crawler"] },
      });
      ...
      customAdapter.updateDataSource("taobao_official_api", { isAvailable: false });
      customAdapter.updateDataSource("taobao_crawler", { isAvailable: false });
      await logDataSourceStatus(customAdapter);
  ...
  describe("Scenario 7: Max fallback sources limit", () => {
    it("should respect maxFallbackSources setting", async () => {
      logStage("INIT", {
        scenario: "Max fallback sources limit",
        settings: { maxFallbackSources: 2 },
      });
  ...
  describe("Scenario 8: Runtime configuration update", () => {
    it("should apply setSourceConfig at runtime", async () => {
      logStage("INIT", { scenario: "Runtime config update" });
      await logDataSourceStatus(adapter);

      logStage("CONFIG_UPDATE", { newPrimary: "taobao_crawler" });
    ...
    it("should apply setSettings at runtime", async () => {
      logStage("INIT", { scenario: "Runtime settings update" });
