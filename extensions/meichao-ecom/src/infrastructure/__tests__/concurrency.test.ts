...
import { InMemoryCooldownManager } from "../cooldown/CooldownManager.js";
...
describe("Concurrency Tests", () => {
  ...
  describe("CooldownManager Concurrency", () => {
    let cooldownManager: InMemoryCooldownManager;

    beforeEach(() => {
      cooldownManager = new InMemoryCooldownManager({
        baseMinutes: 1,
        maxMinutes: 10,
        severeMultiplier: 3,
        probeMinIntervalSeconds: 10,
        probeWindowMinutes: 5,
      });
    });
    ...
    it("should handle concurrent recordError calls", async () => {
      const sourceId = "test-source";

      await runConcurrent(async () => {
        cooldownManager.recordError(sourceId, "rate_limit");
        return cooldownManager.getCooldownState(sourceId).errorCount;
      }, 10);
    ...
    it("should handle concurrent recordSuccess calls", async () => {
      ...
      expect(cooldownManager.getCooldownState(sourceId).errorCount).toBe(0);
    });
    ...
    it("should maintain consistent cooldown state", async () => {
      ...
      await runConcurrent(async () => {
        cooldownManager.recordError(sourceId, "timeout");
  ...
  describe("Degradation Flow Concurrency", () => {
    ...
    it("should handle concurrent cooldown updates", async () => {
      const cooldown = new InMemoryCooldownManager();
      const sources = ["source1", "source2", "source3"];

      const result = await runConcurrent(async () => {
        const source = sources[Math.floor(Math.random() * sources.length)];
        cooldown.recordError(source, "rate_limit");
