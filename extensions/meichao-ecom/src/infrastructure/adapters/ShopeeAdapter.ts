...
import { ShopeeClient } from "../api/shopee/ShopeeClient.js";
import { ShopeeProductApi } from "../api/shopee/ShopeeProductApi.js";
...
export class ShopeeAdapter extends BasePlatformAdapter {
  private productApi?: ShopeeProductApi;
  private apiClient?: ShopeeClient;

  static create(): ShopeeAdapter {
    const dataSources = [
      DataSource.create({
        id: "shopee_official_api",
        platform: "shopee",
        type: "official_api",
        priority: 1,
        costPerCall: 0,
    ...
    return new ShopeeAdapter(adapterConfig);
  }

  private ensureApiInitialized(): ShopeeProductApi {
    if (!this.productApi) {
      this.apiClient = ShopeeClient.fromEnv();
      this.productApi = new ShopeeProductApi(this.apiClient);
    }
      ...
      const result = await this.fetchWithFailover(
        ...
        },
        {
          ...
          onSourceFailure: (sourceId, error) => {
            console.warn(`[ShopeeAdapter] Source ${sourceId} failed: ${error.message}`);
          },
        },
      );
    ...
    try {
      const result = await this.fetchWithFailover(
        ...
        },
        {
          ...
          allowOpenSearch: options?.degradation?.allowOpenSearch,
          onSourceFailure: (sourceId, error) => {
            console.warn(`[ShopeeAdapter] Search source ${sourceId} failed: ${error.message}`);
          },
        },
      );
