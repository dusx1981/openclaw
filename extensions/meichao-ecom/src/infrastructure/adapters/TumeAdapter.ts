...
import { TumeClient } from "../api/tume/TumeClient.js";
import { TumeProductApi } from "../api/tume/TumeProductApi.js";
import { BasePlatformAdapter, type AdapterConfig } from "./BasePlatformAdapter.js";

export class TumeAdapter extends BasePlatformAdapter {
  private productApi?: TumeProductApi;
  private apiClient?: TumeClient;

  static create(): TumeAdapter {
    const dataSources = [
      DataSource.create({
        id: "tume_official_api",
        platform: "tume",
        type: "official_api",
        priority: 1,
        costPerCall: 0,
  ...
  private ensureApiInitialized(): TumeProductApi {
    if (!this.productApi) {
      this.apiClient = TumeClient.fromEnv();
      this.productApi = new TumeProductApi(this.apiClient);
    }
