...
import { JDClient } from "../api/jd/JDClient.js";
import { JDProductApi } from "../api/jd/JDProductApi.js";
...
export class JDAdapter extends BasePlatformAdapter {
  private productApi?: JDProductApi;
  private apiClient?: JDClient;

  static create(): JDAdapter {
    const dataSources = [
      DataSource.create({
        id: "jd_official_api",
        platform: "jd",
    ...
    return new JDAdapter(adapterConfig);
  }

  private ensureApiInitialized(): JDProductApi {
    if (!this.productApi) {
      this.apiClient = JDClient.fromEnv();
      this.productApi = new JDProductApi(this.apiClient);
    }
      ...
      const result = await this.fetchWithFailover(
        ...
        },
        {
          ...
          onSourceFailure: (sourceId, error) => {
            console.warn(`[JDAdapter] Source ${sourceId} failed: ${error.message}`);
          },
        },
      );
    ...
    try {
      const result = await this.fetchWithFailover(
        ...
        },
        {
          ...
          allowOpenSearch: options?.degradation?.allowOpenSearch,
          onSourceFailure: (sourceId, error) => {
            console.warn(`[JDAdapter] Search source ${sourceId} failed: ${error.message}`);
          },
        },
      );
