...
- 使用 TLS (`channels.irc.tls=true`)，除非你故意接受明文传输。
...
- `channels.irc.groupPolicy="open"` 允许未配置的频道（**仍默认需要提及触发**）
...
即使频道被允许（通过 `groupPolicy` + `groups`）且发送者被允许，OpenClaw 在群组上下文中默认**需要提及触发**。

这意味着除非消息包含匹配机器人的提及模式，你可能会看到类似 `drop channel … (missing-mention)` 的日志。

要让机器人在 IRC 频道中**无需提及即可回复**，为该频道禁用提及触发：
...
或允许**所有** IRC 频道（无每频道白名单）且无需提及即可回复：
...
如果你在公共频道中允许 `allowFrom: ["*"]`，任何人都可以触发机器人。
为降低风险，为该频道限制工具。
...
- 旧版无前缀键仍被接受并仅作为 `id:` 匹配。
- 第一个匹配的发送者策略生效；`"*"` 是通配符后备。

关于群组访问与提及触发（及其交互方式）的更多信息，请参阅：[/channels/groups](/channels/groups)。
...
- 如果机器人连接但从未在频道中回复，检查 `channels.irc.groups` **以及**提及触发是否丢弃消息 (`missing-mention`)。如果希望无需 ping 即可回复，为该频道设置 `requireMention:false`。
